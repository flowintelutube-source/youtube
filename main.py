...
def search_cc():
    ...
    req = youtube.search().list(q="story", part="snippet", type="video",
                                videoLicense="creativeCommon", order="viewCount",
                                publishedAfter=date, maxResults=10)
    res = req.execute()
    for v in res["items"]:
        vid = v["id"]["videoId"]
...
def edit():
    video = mp.VideoFileClip("cc.mp4").subclip(0, 55)
    audio = mp.AudioFileClip("voice.mp3")
    mp.VideoFileClip("cc.mp4").subclip(0, 55).set_audio(audio).write_videofile("final.mp4", logger=None)
